...
def turnOnMaintenanceInInstance(instance, file):
    ...
    cmd = 'ssh -p ' + ssh_port + ' ' + host + ' "cd /var/www/' + file + ' && php artisan down"'
    print '========== run cmd =========='
    print cmd
    value_cmd = os.system(cmd)
    print('value_cmd:', value_cmd)
    print 'done------turnOnMaintenanceInInstance'
...
if __name__ == '__main__':
   ...
   for instance in runningAppInstances:
       print '==========runningAppInstances: turn on maintenace mode ' + instance.id + ' START =========='
       turnOnMaintenanceInInstance(instance, 'bitcastle_frontend')

   for instance in runningApiInstances:
       print '==========runningApiInstances: turn on maintenace mode ' + instance.id + ' START =========='
       turnOnMaintenanceInInstance(instance, 'bitcastle_backend')
