<template>
  <div class="container-fluid">
      <modal :show="show" @close="$emit('close')" :css-footer="{ 'is-justify-center': true }" :title="'Add Category Modal'">
        <template slot="body">
          <div class="wrap-login-form">
            <div class="columns">
              <div class="column has-text-centered">
                <h1 style="font-size: 20px; font-weight: bold;">Add Category</h1>
              </div>
            </div>
            <div class="columns">
...
<script>
  ...
  export default {
    name: "StartChatRoomModal",
    extends: CommonHandleModal,
    methods: {
      async promiseRequest() {
        return await this.rf.getRequest('CategoryRequest').addCategory(this.params);
      }
    }
  }
</script>
