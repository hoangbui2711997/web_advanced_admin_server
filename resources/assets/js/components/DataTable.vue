    ...
    <div :style="{width: widthTable}" :class="{'no-data': emptyData}" class="tableContainer">
      <table>
        ...
        <template v-if="!(fetching && orderBy === null)">
          ...
          <template v-if="emptyData">
            <tr>
              <td :colspan="column">
                {{ isSearchAble ? $t('datatable_search_empty') : msgEmptyData || $t('datatable_empty') }}
              </td>
            </tr>
          </template>
          <template v-else>
            <tr v-for="(row, index) in emptyRow" :key="`empty_row_${index}`">
              <td v-for="(col, index) in column">&nbsp;</td>
            </tr>
          </template>
        </template>
        <template v-else>
      ...
      <div class="wrap-footer">
        <div class="select-display">
          <select v-model="limit" @change="filter(params)">
            <option v-for="(item, index) in limits" :value="item" :key="index">{{ item }}</option>
          </select>
        </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        totalRecord: 0,
        lastPage: 0,
        page: Number.isInteger(this.urlPage) ? this.urlPage : 1,
        perPage: 10,
        column: 0,
        fetching: false,
        rows: [],
        params: {
          search_key: this.$route.query.search !== undefined ? this.$route.query.search : null,
          selected_search: this.$route.query.select !== undefined ? this.$route.query.select : '',
        },
        orderBy: null,
        sortedBy: null,
        limits: [5, 10, 25, 50],
        limit: this.limitRows ? this.limitRows : 25,
        keySearch: '',
      };
    },
    computed: {
      isSearchAble () {
        ...
        const keySearch = _.find(keys, key => _.includes(_.toLower(key), 'search'));
        if (keySearch) {
          return !!this.params[keySearch];
        }
        return false;
      },
      emptyRow () {
